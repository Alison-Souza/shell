...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    /* RESPOSTA TAREFA1:
     * O primeiro if é usado para checar se o comando a ser executado é um 
     * 'cd', ou seja, se é um comando para navegação entre diretórios. O 
     * segundo if verifica se o diretório não existe, se não existir, imprime
     * uma mensagem de erro. Sem o primeiro if o comando 'cd' não funcionaria,
     * pois a linha "buf[strlen(buf)-1] = 0;" é quem insere o caractere '\0'
     * no final do nome do diretório. O segundo if é necessário para mostrar ao
     * usuário que não foi encontrado o diretório que ele buscava. */
